...
import { LogOut, User, CreditCard, Clock, Palette, Briefcase, X, ChevronRight } from "lucide-react"
            ...
            >
                <div className="flex w-full h-full gap-4">
                    ...
                    >
                                    ...
                                    <div key={item.id} className="flex flex-col">
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <div
                                                    className={`h-10 w-10 p-1 rounded-full flex items-center justify-center ${active ? 'bg-[#4F4F4F]' : 'bg-white/70'
                                                        }`}
                                                >
                                                    <Icon
                                                        className={`h-4 w-4 ${active ? 'text-white' : 'text-black'}`}
                                                    />
                                                </div>
                        ...
                        <button className="flex items-center gap-3  px-3 py-2 text-sm font-semibold text-white  hover:bg-red-500/10">
                            <div className="bg-red-500 p-2 rounded-full"  >
                                <LogOut className="h-4 w-4 text-white " /></div> Log Out
                        </button>
                    </div>

                    <div
                        className={`flex-1 rounded-2xl relative p-6 scrollbar-hide overflow-y-auto w-[450px] ${activeTab === "personalize" ? 'bg-white' : ''}`}
                        style={{
                            maxHeight: '100%',
                    ...
                    >
                        ...
                        {activeTab === "history" && <HistoryTab activeSubTab={activeHistoryTab} />}
